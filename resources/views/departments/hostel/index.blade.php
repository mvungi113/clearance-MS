                ...
                <tr>
                    ...
                    <td>
                        ...
                        @elseif($request->hostel_status === 'rejected')
                            ...
                            @if($request->hostel_remarks)
                                <br>
                                <small class="text-danger">Reason: {{ $request->hostel_remarks }}</small>
                            @endif
                        @else
                    ...
                    <td>
                        @if($request->hostel_status !== 'verified' && $request->hostel_status !== 'rejected')
                            ...
                            <!-- Reject button triggers reason input -->
                            <button type="button" class="btn btn-danger btn-sm" onclick="showReasonInput({{ $request->id }})" id="reject-btn-{{ $request->id }}">Reject Hostel</button>
                            <form method="POST" action="{{ route('hostel.reject', $request->id) }}" style="display:inline; display:none;" id="reason-form-{{ $request->id }}">
                                @csrf
