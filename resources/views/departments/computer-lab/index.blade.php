                ...
                <tr>
                    ...
                    <td>
                        ...
                        @elseif($request->computer_lab_status === 'rejected')
                            ...
                            @if($request->computer_lab_remarks)
                                <br>
                                <small class="text-danger">Reason: {{ $request->computer_lab_remarks }}</small>
                            @endif
                        @else
                    ...
                    <td>
                        @if($request->computer_lab_status !== 'verified' && $request->computer_lab_status !== 'rejected')
                            ...
                            <button type="button" class="btn btn-danger btn-sm" onclick="showReasonInput({{ $request->id }})" id="reject-btn-{{ $request->id }}">Reject Computer Lab</button>
                            <form method="POST" action="{{ route('computer_lab.reject', $request->id) }}" style="display:inline; display:none;" id="reason-form-{{ $request->id }}">
                                @csrf
