                ...
                <tr>
                    ...
                    <td>
                        ...
                        @elseif($request->hod_status === 'rejected')
                            ...
                            @if($request->hod_remarks)
                                <br>
                                <small class="text-danger">Reason: {{ $request->hod_remarks }}</small>
                            @endif
                        @else
                    ...
                    <td>
                        @if($request->hod_status !== 'verified' && $request->hod_status !== 'rejected')
                            ...
                            <button type="button" class="btn btn-danger btn-sm" onclick="showReasonInput({{ $request->id }})" id="reject-btn-{{ $request->id }}">Reject HoD</button>
                            <form method="POST" action="{{ route('hod.reject', $request->id) }}" style="display:inline; display:none;" id="reason-form-{{ $request->id }}">
                                @csrf
